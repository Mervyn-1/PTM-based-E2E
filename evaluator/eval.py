...
class MultiWozEvaluator(object):
    def __init__(self, reader, cfg):
        ...
        self.mapping_pair_path = './../../../../../data/dataset/multiwoz/utils/mapping.pair'
        ...
        if 'all' in self.cfg.exp_domains:
            ...
            if self.cfg.eval_per_domain:
                ...
                domains = domains + ['restaurant_train', 'restaurant_hotel', 'restaurant_attraction', 'hotel_train',
                                     'hotel_attraction', 'attraction_train', 'restaurant_hotel_taxi',
    ...
    def _get_metric_results(self, data, domain='all', file_list=None):
        ...
        if self.cfg.bspn_mode == 'bspn' or self.cfg.enable_dst:
            ...
            jg_nnnb, sf1_nnnb, sac_nnnb, _, _ = self.dialog_state_tracking_eval(data, file_list, no_name=True,
                                                                                no_book=True)
            metric_result.update({'joint_goal': jg, 'slot_acc': slot_acc, 'slot_f1': slot_f1})
        if self.cfg.bspn_mode == 'bsdx':
            jg_, slot_f1_, slot_acc_, slot_cnt, slot_corr = self.dialog_state_tracking_eval(data, file_list,
                                                                                            bspn_mode='bsdx')
            jg_nn_, sf1_nn_, sac_nn_, _, _ = self.dialog_state_tracking_eval(data, file_list, bspn_mode='bsdx',
                                                                             no_name=True, no_book=False)
        ...
        if dial_num:
            metric_result.update({'act_f1': act_f1, 'success': success, 'match': match, 'bleu': bleu,
                                  'req_slots_acc': req_slots_acc, 'info_slots_acc': info_slots_acc,
                                  'dial_num': dial_num,
            ...
            if self.cfg.bspn_mode == 'bspn' or self.cfg.enable_dst:
                ...
                logging.info('[DST] [not eval name slots] joint goal:%2.1f  slot acc: %2.1f  slot f1: %2.1f' % (
                jg_nn, sac_nn, sf1_nn))
                logging.info('[DST] [not eval book slots] joint goal:%2.1f  slot acc: %2.1f  slot f1: %2.1f' % (
                jg_nb, sac_nb, sf1_nb))
                logging.info('[DST] [not eval name & book slots] joint goal:%2.1f  slot acc: %2.1f  slot f1: %2.1f' % (
                jg_nnnb, sac_nnnb, sf1_nnnb))
            if self.cfg.bspn_mode == 'bsdx':
                logging.info('[BDX] joint goal:%2.1f  slot acc: %2.1f  slot f1: %2.1f  act f1: %2.1f' % (
                jg_, slot_acc_, slot_f1_, act_f1))
                logging.info('[BDX] [not eval name slots] joint goal:%2.1f  slot acc: %2.1f  slot f1: %2.1f' % (
                    ...
                    if requestable == 'reference':
                        if '[value_reference]' in sent_t:
                            if 'booked' in turn['pointer'] or 'ok' in turn[
                                'pointer']:  # if pointer was allowing for that?
