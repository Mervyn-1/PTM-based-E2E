...
def main():
    if not os.path.exists('../../model/ubar/ckpt'):
        os.mkdir('../../model/ubar/ckpt')

    if not os.path.exists('../../model/ubar/ckpt_21'):
        os.mkdir('../../model/ubar/ckpt_21')
    ...
    if args.mode == 'test' or args.mode == 'adjust':
        ...
        if cfg.exp_path in ['', 'to be generated']:
            ...
            # cfg.exp_path = 'ckpt/{}_{}_sd{}_lr{}_bs{}_sp{}_dc{}/'.format('-'.join(cfg.exp_domains),
            #                                                                     cfg.exp_no, cfg.seed, cfg.lr, cfg.batch_size,
            #                                                                     cfg.early_stop_count, cfg.weight_decay_count)
            
            ckpt_path = './ckpt' if 'all' in cfg.exp_domains else './ckpt_Xdomain'
            cfg.exp_path = os.path.join(ckpt_path,'{}_{}_sd{}_lr{}_bs{}_ga{}'.format('-'.join(cfg.exp_domains),
                                                                          cfg.exp_no, cfg.seed, cfg.lr, cfg.batch_size,
                                                                          cfg.gradient_accumulation_steps))
...
#  testing:  python train.py -mode test -cfg eval_load_path=ckpt/all__sd11_lr0.001_bs2_ga8/epoch5_trloss0.80_gpt2/
