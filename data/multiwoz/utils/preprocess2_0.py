...
from config2_0 import global_config as cfg
from db_ops import MultiWozDB
from clean_dataset import clean_slot_values, clean_text


def get_db_values(data_prefix, value_set_path): # value_set.json, all the domain[slot] values in datasets
    processed = {}
    bspn_word = []
    nlp = spacy.load('en_core_web_sm')

    with open(data_prefix + value_set_path, 'r') as f: # read value set file in lower
        value_set = json.loads(f.read().lower())

    with open(data_prefix + 'ontology.json', 'r') as f: # read ontology in lower, all the domain-slot values
    ...
    with open((data_prefix + value_set_path).replace('.json', '_processed.json'), 'w') as f:
        json.dump(processed, f, indent=2) # save processed.json 
    with open(data_prefix + 'multi-woz-processed/bspn_word_collection.json', 'w') as f:
        json.dump(bspn_word, f, indent=2) # save bspn_word

    print('DB value set processed! ')

def preprocess_db(data_prefix, db_paths): # apply clean_slot_values to all dbs
    dbs = {}
    nlp = spacy.load('en_core_web_sm')
    for domain in ontology.all_domains:
        with open(data_prefix + db_paths[domain], 'r') as f: # for every db_domain, read json file 
        ...
        with open((data_prefix + db_paths[domain]).replace('.json', '_processed.json'), 'w') as f:
...
class DataPreprocessor(object):
    def __init__(self, data_prefix):
        ...
        data_path = data_prefix + 'annotated_user_da_with_span_full.json'
        # archive = zipfile.ZipFile(data_path + '.zip', 'r')
        self.convlab_data = json.loads(open(data_prefix + data_path).read().lower())
        # self.convlab_data = json.loads(archive.open(data_path.split('/')[-1], 'r').read().lower())
        self.delex_sg_valdict_path = self.data_prefix + 'multi-woz-processed/delex_single_valdict.json'
        self.delex_mt_valdict_path = self.data_prefix + 'multi-woz-processed/delex_multi_valdict.json'
        self.ambiguous_val_path = self.data_prefix + 'multi-woz-processed/ambiguous_values.json'
        self.delex_refs_path = self.data_prefix + 'multi-woz-processed/reference_no.json'
    ...
    def preprocess_main(self, save_path=None, is_test=False):
        ...
        self.vocab.save_vocab(self.data_prefix + 'multi-woz-processed/vocab')
        with open(self.data_prefix + 'multi-woz-analysis/dialog_acts.json', 'w') as f:
            json.dump(ordered_sysact_dict, f, indent=2)
        with open(self.data_prefix + 'multi-woz-analysis/dialog_act_type.json', 'w') as f:
...
if __name__=='__main__':
    ...
    get_db_values(data_prefix, 'db/value_set.json') # 
    preprocess_db(data_prefix, db_paths)
    dh = DataPreprocessor(data_prefix=data_prefix)
    data = dh.preprocess_main()
    if not os.path.exists(data_prefix + 'multi-woz-processed'):
        os.mkdir(data_prefix + 'multi-woz-processed')

    with open(data_prefix + 'multi-woz-processed/data_for_damd.json', 'w') as f:
